...
/* Initialize the PSoC6 hardware crypto engine for SHA-1, SHA-2, or SHA-512 operation.
 *
 * sha       Pointer to hash context structure (wc_Sha, wc_Sha224, wc_Sha256, wc_Sha384, wc_Sha512).
 * hash_mode Hash mode selector (WC_PSOC_SHA1, WC_PSOC_SHA224, WC_PSOC_SHA256, etc.).
 * init_hash If 1, initializes the hash state; if 0, does not initialize.
...
int wc_Psoc_Sha1_Sha2_Init(void* sha, wc_psoc_hash_sha1_sha2_t hash_mode, int init_hash)
{
    ...
    switch (hash_mode)
    {
#if !defined(NO_SHA) && defined(PSOC_HASH_SHA1)
    case WC_PSOC_SHA1:
...
/* Free resources and clear the register buffer for the PSoC6 hardware crypto engine.
 *
 * No parameters.
 * No return value.
 */
...
int wc_InitSha_ex(wc_Sha* sha, void* heap, int devid)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Sha1_Sha2_Init(sha, WC_PSOC_SHA1, 1);
        // Release the lock
...
int wc_InitSha256_ex(wc_Sha256* sha, void* heap, int devid)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Sha1_Sha2_Init(sha, WC_PSOC_SHA256, 1);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
    return ret;
...
int wc_InitSha224_ex(wc_Sha224* sha, void* heap, int devid)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Sha1_Sha2_Init(sha, WC_PSOC_SHA224, 1);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
    return ret;
...
int wc_InitSha384_ex(wc_Sha384* sha, void* heap, int devid)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Sha1_Sha2_Init(sha, WC_PSOC_SHA384, 1);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
    return ret;
...
int wc_InitSha512_ex(wc_Sha512* sha, void* heap, int devid)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Sha1_Sha2_Init(sha, WC_PSOC_SHA512, 1);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
    return ret;
...
int wc_InitSha512_224_ex(wc_Sha512* sha, void* heap, int devid)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Sha1_Sha2_Init(sha, WC_PSOC_SHA512_224, 1);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
    return ret;
...
int wc_InitSha512_256_ex(wc_Sha512* sha, void* heap, int devid)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Sha1_Sha2_Init(sha, WC_PSOC_SHA512_256, 1);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
    return ret;
...
/* Initialize the state for a SHA-3 hash operation using the PSoC6 hardware crypto engine.
 *
...
/* Update the SHA-3 hash state with input data using the PSoC6 hardware crypto engine.
 *
 * sha3   wc_Sha3 object holding state.
...
/* Finalize the SHA-3 hash operation and produce the digest using the PSoC6 hardware crypto engine.
 *
 * sha3    wc_Sha3 object holding state.
...
/* Squeeze output blocks from a SHAKE state using the PSoC6 hardware crypto engine.
 *
