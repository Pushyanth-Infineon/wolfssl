...
#if !defined(STM32_HASH_SHA3) && !defined(PSOC_HASH_SHA3)
/* An array of values to XOR for block operation. */
static const word64 hash_keccak_r[24] =
{
...
static int wc_Sha3Update(wc_Sha3* sha3, const byte* data, word32 len, byte p)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Sha3_Update(sha3, data, len, p);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
...
static int wc_Sha3Final(wc_Sha3* sha3, byte* hash, byte p, byte len)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Sha3_Final(sha3, 0x06, hash, p, len);
        if (ret == 0) {
            // Initialize hash state for SHA-3 operation
            ret = wc_Psoc_Sha3_Init(sha3);
        }
...
int wc_Shake128_Update(wc_Shake* shake, const byte* data, word32 len)
{
    ...
    if (ret == 0) {
        // Perform SHA3 on the input data and update the hash state
        ret = wc_Psoc_Sha3_Update(shake, data, len, WC_SHA3_128_COUNT);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
...
int wc_Shake128_Final(wc_Shake* shake, byte* hash, word32 hashLen)
{
    ...
    if (ret == 0) {
        // Finalize SHA3 operations and produce digest
        ret = wc_Psoc_Sha3_Final(shake, 0x1f, hash, WC_SHA3_128_COUNT, hashLen);
        if (ret == 0) {
            // Initialize hash state for SHA-3 operation
            ret = wc_Psoc_Sha3_Init(shake);
        }
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
...
int wc_Shake128_Absorb(wc_Shake* shake, const byte* data, word32 len)
{
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            byte hash[1];
            // Finalize SHA3 operations and produce digest
            ret = wc_Psoc_Sha3_Final(shake, 0x1f, hash, WC_SHA3_128_COUNT, 0);
        }
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
...
int wc_Shake128_SqueezeBlocks(wc_Shake* shake, byte* out, word32 blockCnt)
{
    ...
    if (ret == 0) {
        ...
        ret = wc_Psoc_Shake_SqueezeBlocks(shake, out, blockCnt);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
...
int wc_Shake256_Update(wc_Shake* shake, const byte* data, word32 len)
{
    ...
    if (ret == 0) {
        // Perform SHA3 on the input data and update the hash state
        ret = wc_Psoc_Sha3_Update(shake, data, len, WC_SHA3_256_COUNT);
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
...
int wc_Shake256_Final(wc_Shake* shake, byte* hash, word32 hashLen)
{
    ...
    if (ret == 0) {
        // Finalize SHA3 operations and produce digest
        ret = wc_Psoc_Sha3_Final(shake, 0x1f, hash, WC_SHA3_256_COUNT, hashLen);
        if (ret == 0) {
            // Initialize hash state for SHA-3 operation
            ret = wc_Psoc_Sha3_Init(shake);
        }
...
int wc_Shake256_Absorb(wc_Shake* shake, const byte* data, word32 len)
{
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            byte hash[1];
            // Finalize SHA3 operations and produce digest
            ret = wc_Psoc_Sha3_Final(shake, 0x1f, hash, WC_SHA3_256_COUNT, 0);
        }
        // Release the lock
        wolfSSL_CryptHwMutexUnLock();
    }
